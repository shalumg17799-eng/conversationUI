...
// Loads synthetic data into all 18 tables
...
async function insertData(tableId, rows) {
  ...
  try {
    ...
    const [job] = await bigquery.dataset(dataset).table(tableId).insert(rows);
...
async function loadDimensionTables() {
  ...
  const markets = marketsData.map(market => ({
    market_id: market.market_id || `MKT-${market.market_name.toUpperCase().replace(/\s+/g, '-')}`,
    market_name: market.market_name,
    description: market.description || `${market.market_name} region`
  }));
