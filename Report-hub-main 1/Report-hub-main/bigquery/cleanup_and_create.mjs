// Cleanup old dataset and create new one
import { BigQuery } from '@google-cloud/bigquery';

const bigquery = new BigQuery({
  projectId: 'data-practice-472314',
  credentials: {
    client_email: 'bigquery-backend-dp@data-practice-472314.iam.gserviceaccount.com',
    private_key: "-----BEGIN PRIVATE KEY-----\\nDrlls5cFfxGFFQglnbrTySYC\n-----END PRIVATE KEY-----\n"
  }
});

const oldDatasetId = 'telecom_demo';
const newDatasetId = 'report_hub_demo';

async function cleanupAndCreate() {
  try {
    console.log('🧹 Cleaning up old datasets...');
    
    // Delete old dataset
    try {
      await bigquery.dataset(oldDatasetId).delete();
      console.log(`✅ Deleted old dataset: ${oldDatasetId}`);
    } catch (error) {
      console.log(`ℹ️  Old dataset ${oldDatasetId} doesn't exist or couldn't be deleted`);
    }
    
    // Delete existing new dataset
    try {
      await bigquery.dataset(newDatasetId).delete();
      console.log(`✅ Deleted existing dataset: ${newDatasetId}`);
    } catch (error) {
      console.log(`ℹ️  Dataset ${newDatasetId} doesn't exist`);
    }
    
    console.log('✅ Cleanup completed!');
    
  } catch (error) {
    console.error('❌ Error during cleanup:', error.message);
  }
}

// Run cleanup
cleanupAndCreate();
