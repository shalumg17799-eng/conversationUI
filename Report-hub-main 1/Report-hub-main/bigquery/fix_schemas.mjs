...
// Fixes BigQuery table schemas to match synthetic data structure
...
async function recreateTable(tableId, schema) {
  try {
    console.log(`🔄 Recreating table: ${tableId}`);
    ...
    console.log(`   ✅ Successfully recreated ${tableId}`);
    return table;
  } catch (error) {
    console.error(`   ❌ Error recreating ${tableId}:`, error.message);
    throw error;
  }
}

// Load data into table
async function loadData(tableId, rows) {
...
async function fixDimDevices() {
  ...
  await recreateTable('dim_devices', schema);
  await loadData('dim_devices', syntheticData);
}
...
async function fixDimTerritories() {
  ...
  await recreateTable('dim_territories', schema);
  await loadData('dim_territories', syntheticData);
}
...
async function fixDimOutlets() {
  ...
  await recreateTable('dim_outlets', schema);
  await loadData('dim_outlets', syntheticData);
}
...
async function fixCatalogReports() {
  ...
  await recreateTable('catalog_reports', schema);
  await loadData('catalog_reports', transformedData);
}
...
async function fixCatalogDatasets() {
  ...
  await recreateTable('catalog_datasets', schema);
  await loadData('catalog_datasets', transformedData);
}
...
async function fixChurnMonthly() {
  ...
  await recreateTable('churn_monthly', schema);
  await loadData('churn_monthly', syntheticData);
}
...
async function fixTakeRateTrend() {
  ...
  await recreateTable('take_rate_monthly_trend', schema);
  await loadData('take_rate_monthly_trend', syntheticData);
}
...
async function fixMarketSegmentDistribution() {
  ...
  await recreateTable('market_segment_distribution', schema);
  await loadData('market_segment_distribution', syntheticData);
}
...
async function fixSegmentPerformanceTrend() {
  ...
  await recreateTable('segment_performance_trend', schema);
  await loadData('segment_performance_trend', syntheticData);
}
...
async function fixPerformanceByRegion() {
  ...
  await recreateTable('performance_by_region', schema);
  await loadData('performance_by_region', syntheticData);
}
...
async function fixRevenueByDeviceGroup() {
  ...
  await recreateTable('revenue_by_device_group', schema);
  await loadData('revenue_by_device_group', syntheticData);
}
