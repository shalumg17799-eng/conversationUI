// Fact-checker for chart scenes (canonical / frontend copy).
//
// A report component carries two descriptions of the same numbers: the prose the
// report author wrote (props.explanation) and the chart array itself (props.data).
// The video renders the ARRAY — the bars, the peak spotlight, the bullets and the
// narration are all computed from it. When the prose disagrees with the array
// (stale text, a different slice, a different aggregation) the slide ends up
// showing one claim while the voiceover speaks another, which is exactly the
// "narration doesn't match the screen" failure.
//
// So: the rendered data is the single source of truth for a video scene. This
// module derives the authoritative facts from a chart and audits arbitrary prose
// against them, so a contradicted explanation can be dropped BEFORE it reaches
// the slide or the narration writer.
//
// The backend render owns a logically identical copy at
// backend/src/services/chartFacts.ts (the two tsconfig roots can't share one
// module — same constraint as timing.ts / sceneTiming.ts). scripts/test_chartFacts.ts
// fails the build if they drift. Change one → change both.
