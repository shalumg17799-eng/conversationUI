...
app.post('/api/conversational/stream', async (req: Request, res: Response) => {
  const { query, skipClarification, clarificationHistory, priorContext } = req.body;
  ...
  try {
    await runStreamingPipeline(query, send, !!skipClarification, clarificationHistory ?? [], priorContext);
