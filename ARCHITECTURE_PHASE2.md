...
| 5 | Adaptive UI (Conversational Personalization) | ✅ Delivered — NL layout directives live (typed contract: 4 targets × 4 ops, backend intent detection + Ajv validation + telemetry, frontend prefs store persisted to localStorage). All four surfaces render: right panel (move/resize/hide), left panel (resize/hide with chat reflow), nav rail (hide), chat panel (hide), global density. Repositioning of the secondary surfaces is intentionally a no-op — only the report panel is freely repositionable. |
